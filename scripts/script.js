...
const listOfEndpoint = ["users", "users/me", "cinemas", "movies", "reservations", "showtimes"]
...
export default function () {
    ...
    if (!isRegister) {
        ...
        group('User Register', () => {
            const createUserRes = http.post(
                `${BASE_URL}/users`,
                JSON.stringify({ username: USERNAME, password: PASSWORD, name: "jikirzzz", role: "superadmin", email: "jikirzzz@std.stei.itb.ac.id", phone: "081220524864" }),
    ...
    group('Delete', () => {
        const logOutRes = http.delete(
            `${BASE_URL}/users/testDelete/${idUser}`,
            { headers: { 'Content-Type': 'application/json' } }
        );
        check(logOutRes, {
            'Logout Successful': (r) => r.status === 200,
